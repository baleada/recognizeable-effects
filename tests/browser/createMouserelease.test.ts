...
suite(`recognizes mouserelease`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    const listenable = new window.Listenable<MousereleaseTypes, MousereleaseMetadata>(
      'recognizeable' as MousereleaseTypes,
      { recognizeable: { effects: window.effects.createMouserelease() } }
    );
...
suite(`respects minDuration option`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    const listenable = new window.Listenable<MousereleaseTypes, MousereleaseMetadata>(
      'recognizeable' as MousereleaseTypes, 
      { recognizeable: { effects: window.effects.createMouserelease({ minDuration: 1000 }) } }
    );
...
suite(`respects minDistance option`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    const listenable = new window.Listenable<MousereleaseTypes, MousereleaseMetadata>(
      'recognizeable' as MousereleaseTypes,
      { recognizeable: { effects: window.effects.createMouserelease({ minDistance: 101 }) } }
    );
...
suite(`calls hooks`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    ...
    const listenable = new window.Listenable<MousereleaseTypes, MousereleaseMetadata>(
      ...
      {
        recognizeable: {
          effects: window.effects.createMouserelease({
