...
suite(`recognizes mousepress`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    const listenable = new window.Listenable<MousepressTypes, MousepressMetadata>(
      ...
      { recognizeable: { effects: window.effects.createMousepress() } }
    );
...
suite(`respects minDuration option`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    const listenable = new window.Listenable<MousepressTypes, MousepressMetadata>(
      'recognizeable' as MousepressTypes, 
      { recognizeable: { effects: window.effects.createMousepress({ minDuration: 1000 }) } }
    );
...
suite(`respects minDistance option`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    const listenable = new window.Listenable<MousepressTypes, MousepressMetadata>(
      'recognizeable' as MousepressTypes,
      { recognizeable: { effects: window.effects.createMousepress({ minDistance: 101 }) } }
    );
...
suite(`calls hooks`, async ({ playwright: { page, reloadNext } }) => {
  await page.evaluate(async () => {
    ...
    const listenable = new window.Listenable<MousepressTypes, MousepressMetadata>(
      ...
      {
        recognizeable: {
          effects: window.effects.createMousepress({
